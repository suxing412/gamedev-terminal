// 契约一致性.js —— 各家适配器必须过的同一套比对：同一张夹具单，各家交回的证据包**形状一致**，
// 给了单的话**机判初检结论也一致**。
//
...
//
// 09-04 评审甲-18：说明书写了「初检结论一致」，首版只比形状。现在 比对(包们, 单) 带单就对每包跑
// 领域/校验.初检，结论不同的点名——同形但一家 退出=error 一家 completed，形状一致、结论不一致，
// 那才是「同一张单在不同后端得到不同结论」的本体。
//
...
/**
 * 比对一组包。以第一个为基准，逐个比形状；每个包都要过 验包；给了 单 再比初检结论。
 * @param 包们  [{ 家, 包 }]
 * @param 单    可选；给了就对每包跑 初检，结论不一致算差异
 * @returns { 一致, 差异: [{ 家, 因 }] }
 */
function 比对(包们, 单) {
  const 差异 = [];
  ...
  if (单) {
    const 基检 = 校验.初检(基.包, 单).初检;
    for (const { 家, 包 } of 表.slice(1)) {
      const 检 = 校验.初检(包, 单).初检;
      if (检 !== 基检) 差异.push({ 家, 因: `初检结论与 ${基.家} 不一致：${检} vs ${基检}——同一张单在不同后端得到了不同结论` });
    }
  }
