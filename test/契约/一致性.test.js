...
test('契⑥ 带单比对：两包同形，一家 退出=error 一家 completed → 初检结论不一致，点名（甲-18）', async () => {
  const a = E.攒包(await 假适配器跑(进));
  const 料 = await 假适配器跑(进); 料.结果 = { ...料.结果, 退出: 'error' };
  const b = E.攒包(料);
  const 单 = { id: 'FX-1', 性质: '新建', 职能: '程序' };
  assert.strictEqual(一致.比对([{ 家: '甲', 包: a }, { 家: '乙', 包: b }]).一致, true, '不带单只比形状：同形');
  const r = 一致.比对([{ 家: '甲', 包: a }, { 家: '乙', 包: b }], 单);
  assert.strictEqual(r.一致, false);
  assert.ok(r.差异.some((d) => d.家 === '乙' && /初检结论/.test(d.因)), JSON.stringify(r.差异));
});
