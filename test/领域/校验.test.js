...
test('检⑪ 项管写了 预计产出，执行者没交那个文件 → 打回（下游单的进项指着它）', () => {
  const r = J.初检(好包({ 改动: { 文件: ['Assets/SLG/别的.cs'], diff: 'd' } }), { 性质: '新建', 职能: '程序', 预计产出: { 资产: 'Assets/SLG/x.cs' } }, 卷);
  assert.strictEqual(r.初检, '打回');
  违(r, '预计产出 Assets/SLG/x.cs 没交');
  assert.strictEqual(J.初检(好包(), { 性质: '新建', 职能: '程序', 预计产出: { 资产: 'Assets/SLG/x.cs' } }, 卷).初检, '通过');
});
